...
createTableGSEA = function(x, db){
  ...
  list = lapply(list,function(x) {x[order(x$NOM.p.val),]})
...
write.table(createTableGSEA(pattern_CTL, db = GObp),"summary_GSEA_CTL_GObp.tsv",sep = '\t',
  col.names = T, row.names = F, quote = F)
write.table(createTableGSEA(pattern_CTL, db = GOcc),"summary_GSEA_CTL_GOcc.tsv",sep = '\t',
  col.names = T, row.names = F, quote = F)
